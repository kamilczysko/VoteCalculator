...
@Component public class RegisterPanel {
	...
	private boolean registerUser() {
		...
		if (firstNameField.getText().isEmpty() || secondNameField.getText().isEmpty() || !(passwordField.getText()
				.equals(confirmPasswordField.getText()))) {
			warning("Fill all fields properly!");
			return false;
		}

		String peselHash = Hashing.sha256().hashString(peselField.getText(), StandardCharsets.UTF_8).toString();
	...
	public void initialize() {
		...
		if (roles == null) {
			logger.error(
					"Database probably has been not initialized properly. Run application from command line with argument\"first\"");
			System.exit(0);
		}
	}
