...
@Service public class CandidateService implements CommandLineRunner {
	...
	@Secured("ROLE_unvoted") public void vote(Candidate candidate) {
	...
	private void initCandidatesInDB() throws IOException {
		...
		for (Party p : partiesSaved)
			map.put(p.getParty(), p);
	...
	private Candidates getCandidatesFromServer() throws IOException {
		...
		ResponseEntity<String> response = restTemplate
				.exchange("http://webtask.future-processing.com:8069/candidates", HttpMethod.GET, entity, String.class);

		Pattern p = Pattern
				.compile("(" + Pattern.quote("<candidates>") + "(.*?)" + Pattern.quote("</candidates>") + ")");
