...
@Configuration public class BeanConfiguration {
	...
	@Bean @Scope("prototype") public Disallowed getDisallowed() throws IOException {
		...
		Pattern p = Pattern
				.compile("(" + Pattern.quote("<disallowed>") + "(.*?)" + Pattern.quote("</disallowed>") + ")");
		...
		Disallowed disallowed = new XmlMapper(module).readValue(result, Disallowed.class);
	...
	private ResponseEntity<String> getDisallowedFromServer() {
		...
		ResponseEntity<String> response = restTemplate
				.exchange("http://webtask.future-processing.com:8069/blocked", HttpMethod.GET, entity, String.class);
